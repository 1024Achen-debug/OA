                    ...
                    <div class="row">
                        <div class="hidden-xs hidden-sm col-md-3">
                            <div class="btn-group">
                                ...
                                <button type="button" class="btn btn-default light">
                                    <i class="fa fa-plus"
                                       onclick="javascript:window.location.href='${pageContext.request.contextPath}/wei_bao/to_add';"></i>
                                </button>
                            </div>
                        </div>
                            ...
                            <tr class="message-unread">
                                ...
                                <td>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_CREATED || cv.status==Contant.CLAIMVOUCHER_BACK}">
                                        <a href="${pageContext.request.contextPath}/wei_bao/to_update?id=${cv.id}">修改</a>
                                        <a href="${pageContext.request.contextPath}/wei_bao/submit?id=${cv.id}">提交</a>
                                    </c:if>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_SUBMIT || cv.status==Contant.CLAIMVOUCHER_RECHECK}">
                                        <a href="${pageContext.request.contextPath}/wei_bao/to_check?id=${cv.id}">审核</a>
                                    </c:if>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_APPROVED}">
                                        <a href="${pageContext.request.contextPath}/wei_bao/to_check?id=${cv.id}">打款</a>
                                    </c:if>
                                    <a href="${pageContext.request.contextPath}/wei_bao/detail?id=${cv.id}">详细信息</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
